...
void wrapCurrentLine(char line[], int numberOfCharsPerLine) {
    ...
    while(line[currentChar] != ' ' && currentChar >= 0) {
        currentChar--;
    }
    ...
    memcpy(outputLine, line, sizeof(char) * (currentChar + 1));
    char newLine[MAX_LINE_SIZE];
    memset(newLine, 0, MAX_LINE_SIZE);
    memcpy(newLine, line + sizeof(char) * (currentChar + 1), sizeof(char) * (lineLength - currentChar - 1));
    printf("%s\n", outputLine);
